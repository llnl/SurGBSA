...
class EGNN(nn.Module):
    ...
    def forward(self, feats, coors, edges=None, mask=None, adj_mat=None):
        ...
        # Calculate the relative position matrix, with dimensions (b, n, n, 1)
        ...
        if use_nearest:
            ...
            # Returns the `num_nearest` smallest values ​​and their corresponding indices. Note that `num_nearest` should be less than or equal to `n`.
            nbhd_ranking, nbhd_indices = ranking.topk(
                num_nearest, dim=-1, largest=False
            )

            # 默认valid_radius为无穷大，类似于r-ball的设计
            # The default valid_radius is infinity, similar to the design of an r-ball.
...
#             # # Output graph-level features. Although masks need to be considered, the effect is better without masks...
#             return coors, self.out(feats)
#         else:
#             # 输出维度为(B,N,3), (B,N,dim)
#             return coors, feats

#     # def gen_trajectory(self, feats, coors, prompt=None, adj_mat=None, edges=None, mask=None, return_coor_changes=False, n_samples=1):

#     # import pdb
#     # pdb.set_trace()
#     # traj = torch.zeros_like(coors, dtype=torch.float32)

#     # traj = traj.unsqueeze(dim=0).repeat(n_samples, 1, 1)

#     # coors_t_0 = coors
#     # for i in range(n_samples):
#     # import pdb
#     # pdb.set_trace()
#     # traj[i] = self.forward(coors_only=True, feats=feats, coors=coors_t_0, prompt=prompt, adj_mat=adj_mat, edges=edges, mask=mask, return_coor_changes=return_coor_changes)
#     # coors_t_0 = traj[i]
...
class EGNN_Network(nn.Module):
    def __init__(
        ...
        *,
        ...
        dim,
        num_tokens=None,
        num_residue_tokens=None,  # NEW: number of residue types
        residue_dim=None,  # NEW: dimension for residue embedding
        num_edge_tokens=None,
        num_positions=None,
        edge_dim=0,
        num_adj_degrees=None,
        ...
        **kwargs,
        ...
        # NEW: Residue type embedding
        self.use_residue_emb = exists(num_residue_tokens) and exists(residue_dim)
        if self.use_residue_emb:
            self.residue_emb = nn.Embedding(num_residue_tokens, residue_dim)
            # If using residue embeddings, total input to EGNN layers will be dim + residue_dim
            # We'll project this back to dim
    ...
    def forward(
        ...
        feats,
        coors,
        res_feats=None,  # NEW: residue indices
        prompt=None,
        adj_mat=None,
        edges=None,
        mask=None,
        ...
        # NEW: Embed and concatenate residue information
        if self.use_residue_emb and exists(res_feats):
            res_embedded = self.residue_emb(res_feats)
            # Concatenate atom features with residue features
...
class predictor(nn.Module):
    def __init__(self, dim, dropout=0.1):
        ...
        self.out = nn.Sequential(
            nn.Linear(dim, dim), nn.ReLU(), nn.Dropout(p=dropout), nn.Linear(dim, 1)
        )
    ...
    def forward(self, feats, mask=None):
        if exists(mask):
            ...
            return self.out(
                torch.sum(feats, dim=-2) / torch.sum(mask, dim=-1, keepdim=True)
            )
